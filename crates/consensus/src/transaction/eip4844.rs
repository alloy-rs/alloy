...
impl TxEip4844Variant {
    ...
    pub fn encode_with_signature(
        ...
        let payload_length = match self {
            Self::TxEip4844(tx) => {
                tx.fields_len() + signature.rlp_rs_len() + signature.v().length()
            }
            Self::TxEip4844WithSidecar(tx) => {
                let payload_length =
                    tx.tx().fields_len() + signature.rlp_rs_len() + signature.v().length();
...
impl SignableTransaction<Signature> for TxEip4844Variant {
    ...
    fn into_signed(self, signature: Signature) -> Signed<Self> {
        let payload_length =
            1 + self.fields_len() + signature.rlp_rs_len() + signature.v().length();
...
impl TxEip4844WithSidecar {
    ...
    pub fn encode_with_signature_fields(&self, signature: &Signature, out: &mut dyn BufMut) {
        let inner_payload_length =
            self.tx.fields_len() + signature.rlp_rs_len() + signature.v().length();
