...
impl TxDeposit {
    ...
    ///
    /// This __does not__ decode the RLP header, and assumes the RLP header has already been
    /// decoded.
    ///
    /// This __does not__ expect the bytes to start with a list header and include a signature.
...
mod tests {
    ...
    fn test_rlp_roundtrip() {
        ...
        let tx_a = TxEnvelope::decode(&mut bytes.as_ref()).unwrap();
        let tx_b = TxEnvelope::decode(&mut &bytes[..]).unwrap();
    ...
    fn test_encode_with_and_without_header() {
        ...
        tx_deposit.encode(&mut buffer_without_header);
    ...
    fn encode_decode_eip1559() {
        ...
        let decoded = TxDeposit::decode_signed_fields(&mut &buf[..]).unwrap();
