...
use alloy_provider::{MulticallItem, PendingTransactionBuilder, Provider};
...
impl<T, P: Provider<N>, C: SolCall, N: Network> MulticallItem for SolCallBuilder<T, P, C, N> {
    ...
    fn input(&self) -> Bytes {
        self.request.input().expect("`input` not set for the `SolCallBuilder`").clone()
    }
}
...
mod tests {
    ...
    async fn test_multicall() {
        ...
        let (_block_num, (_ts, _bal)) = multicall.aggregate().await.unwrap();
    }
