...
use alloy_primitives::{Address, Bytes, Log, B256, U64};
...
mod tests {
    ...
    fn test_eth_simulate_v1_account_not_precompile() {
        ...
        let sim_opts: SimOpts = serde_json::from_value(request_json["params"][0].clone()).unwrap();
        ...
        assert_eq!(sim_opts.validation, true);
        assert_eq!(sim_opts.block_state_calls.len(), 2);

        let block_state_call_1 = &sim_opts.block_state_calls[0];
        assert!(block_state_call_1.state_overrides.get(&address_1).is_some());
        assert_eq!(
        ...
        assert!(block_state_call_2.state_overrides.get(&address_1).is_some());
        ...
        let error_response: SimulateV1Error =
            serde_json::from_value(error_response_json["error"].clone()).unwrap();

        assert_eq!(error_response.code, -32000);
        assert_eq!(
            error_response.message,
            "account 0xC000000000000000000000000000000000000000 is not a precompile"
        );
    }
}
