...
pub struct AccountDiff {
    ...
    #[serde(with = "delta_u64_serde")]
    pub nonce: Delta<u64>,
    /// All touched/changed storage values
    pub storage: BTreeMap<B256, Delta<B256>>,
}

// Custom serialization/deserialization for Delta<u64>
mod delta_u64_serde {
    use super::{ChangedType, Delta, U64};
    use serde::{Deserialize, Deserializer, Serialize, Serializer};

    pub fn serialize<S>(delta: &Delta<u64>, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        match delta {
            Delta::Unchanged => Delta::<U64>::Unchanged.serialize(serializer),
            Delta::Added(val) => Delta::<U64>::Added(U64::from(*val)).serialize(serializer),
            Delta::Removed(val) => Delta::<U64>::Removed(U64::from(*val)).serialize(serializer),
            Delta::Changed(ChangedType { from, to }) => {
                Delta::<U64>::Changed(ChangedType {
                    from: U64::from(*from),
                    to: U64::from(*to),
                })
    ...
    pub fn deserialize<'de, D>(deserializer: D) -> Result<Delta<u64>, D::Error>
    where
        D: Deserializer<'de>,
    {
        let delta_u64 = Delta::<U64>::deserialize(deserializer)?;
        match delta_u64 {
            Delta::Unchanged => Ok(Delta::Unchanged),
            Delta::Added(val) => Ok(Delta::Added(val.to::<u64>())),
            Delta::Removed(val) => Ok(Delta::Removed(val.to::<u64>())),
            Delta::Changed(ChangedType { from, to }) => {
                Ok(Delta::Changed(ChangedType {
                    from: from.to::<u64>(),
                    to: to.to::<u64>(),
                }))
            }
...
mod tests {
  ...
  "transactionHash": "0xe56a5e7455c45b1842b35dbcab9d024b21870ee59820525091e183b573b4f9eb"
    ...
    #[test]
    fn test_delta_u64_serde() {
        // Test Delta::Unchanged
        let delta_unchanged: Delta<u64> = Delta::Unchanged;
        let serialized = serde_json::to_string(&delta_unchanged).unwrap();
        assert_eq!(serialized, "\"=\"");
        let deserialized: Delta<u64> = serde_json::from_str(&serialized).unwrap();
        assert!(matches!(deserialized, Delta::Unchanged));

        // Test Delta::Added
        let delta_added: Delta<u64> = Delta::Added(123);
        let serialized = serde_json::to_string(&delta_added).unwrap();
        assert_eq!(serialized, "{\"+\":123}");
        let deserialized: Delta<u64> = serde_json::from_str(&serialized).unwrap();
        assert!(matches!(deserialized, Delta::Added(123)));

        // Test Delta::Removed
        let delta_removed: Delta<u64> = Delta::Removed(456);
        let serialized = serde_json::to_string(&delta_removed).unwrap();
        assert_eq!(serialized, "{\"-\":456}");
        let deserialized: Delta<u64> = serde_json::from_str(&serialized).unwrap();
        assert!(matches!(deserialized, Delta::Removed(456)));

        // Test Delta::Changed
        let delta_changed: Delta<u64> = Delta::Changed(ChangedType { from: 100, to: 200 });
        let serialized = serde_json::to_string(&delta_changed).unwrap();
        assert_eq!(serialized, "{\"*\":{\"from\":100,\"to\":200}}");
        let deserialized: Delta<u64> = serde_json::from_str(&serialized).unwrap();
        if let Delta::Changed(ChangedType { from, to }) = deserialized {
            assert_eq!(from, 100);
            assert_eq!(to, 200);
        } else {
            panic!("Expected Delta::Changed, got {:?}", deserialized);
        }

        // Test within AccountDiff
        let account_diff = AccountDiff {
            balance: Delta::Unchanged,
            code: Delta::Unchanged,
            nonce: Delta::Added(42),
            storage: BTreeMap::new(),
        };
        
        let serialized = serde_json::to_string(&account_diff).unwrap();
        let deserialized: AccountDiff = serde_json::from_str(&serialized).unwrap();
        
        assert!(matches!(deserialized.balance, Delta::Unchanged));
        assert!(matches!(deserialized.code, Delta::Unchanged));
        assert!(matches!(deserialized.nonce, Delta::Added(42)));
    }
