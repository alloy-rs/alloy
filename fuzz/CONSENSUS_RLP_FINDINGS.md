...
It checks stable canonicalization instead of injectivity for the documented `TxLegacy` and bare
typed-network compatibility forms. Generated corpus entries are canonical values; no known
crashing input is embedded in the target.
...
| `network_decode` accepts an unwrapped typed transaction | top-level `BasicTxEnvelope` injectivity | Retained for backwards compatibility and documented in [alloy#4217](https://github.com/alloy-rs/alloy/pull/4217) | Intentional compatibility behavior; the oracle requires stable canonicalization to wrapped network encoding |
| `TxLegacy::decode` accepts non-list and non-zero-sentinel forms | unsigned legacy transaction injectivity | Retained for backwards compatibility and documented in [alloy#4217](https://github.com/alloy-rs/alloy/pull/4217) | Intentional compatibility behavior; the oracle requires stable canonicalization |
...
The only intentional injectivity exclusions are the compatibility forms documented above.
Explicitly type-`0x00`-prefixed legacy inputs are invalid EIP-2718 encodings and remain in the
transaction-envelope and pooled-envelope oracles.
